    ...
    try {
      ...
      if (employeeData.role.toString() !== '2') {
        ...
        for (let i = 0; i < onScheduleMemberId.length; i++) {
          ...
          const nextSchedules = scheduleData.filter(val => {
            ...
            if (val.shift.schedule_shift.is_tommorow) {
              endTime = new Date(new Date(endTime).setHours(new Date(endTime).getHours() + 24));
            }
            return date < endTime;
          });
      ...
      const payload = Object.assign(
        {},
        {
          ...
          is_has_email: employeeData.user.email !== null && employeeData.user.email !== '',
          ...
          is_email_confirmed: employeeData.user.is_email_confirmed,
          demo: {
